...
- **Ask the AI** panels (Visualization and Inference) for short explanations of plots and results when a Groq API key is configured.
...
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **One categorical column**         | - **Bar chart** — counts or proportions; **tilted x-axis labels** when the x-axis has **5–9** distinct categories; **horizontal bars** default **on** when there are more than **10** categories (toggle with **Flip coordinates**). - **Pie chart**. - Optional **frequency table** (with bar or pie).                                                                                                           |
| **Two categorical columns**        | - **Side-by-side (dodge)** grouped bars. - **Stacked** grouped bars. - **Proportional (fill)** grouped bars. - Shared layout with one-category bars: **tilted x-axis labels** when x has **5–9** categories; **horizontal bars** default **on** when there are more than **10** categories on x (**Flip coordinates** overrides).                                                                                 |
| **One numeric column**             | - **Histogram** — bin width, optional boundary, density scaling. - **Boxplot**.                                                                                                                                                                                                                                                                                                                                   |
| **Numeric vs categorical columns** | - **Side-by-side boxplot** — auto **horizontal** layout when there are more than **10** x groups. - **Faceted histogram**. - **Bar chart** — same **tilted labels** when categorical x has **5–9** categories; **horizontal bars** default **on** when there are more than **10** categories on x (**Flip coordinates**). - **Pie chart**. - **Line plot** — optional cap on x-axis label count for crowded axes. |
| **Two numeric columns**            | - **Scatter plot** — optional **linear regression line** and displayed equation.                                                                                                                                                                                                                                                                                                                                  |
...
| Hypothesis test               | Role (short)                                                                                                 |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------ |
| **One proportion**            | Single categorical column vs a null proportion.                                                              |
| **Two proportions**           | Compare two groups on a binary outcome.                                                                      |
| **Chisq for goodness of fit** | One categorical column vs a user-specified null category probabilities (including **Reset to uniform 1/k**). |
