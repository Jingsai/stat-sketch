...
def is_categorical(series: pd.Series, max_unique_ratio: float = 0.2, max_unique: int = 50) -> bool:
    """
...
st.sidebar.header("2) Basic options")
...
with tabs[0]:
    ...
    if mode == "One categorical variable":
        ...
        if show_percent:
            ...
            p = (
                ggplot(d, aes(x=x, y="after_stat(prop)", group=1))
                + geom_bar()
...
with tabs[1]:
    ...
    if mode == "One numeric variable":
        ...
        binwidth = st.number_input("Histogram binwidth (leave as 0 for auto bins)", min_value=0.0, value=0.0, step=1.0)
        boundary = st.number_input("Histogram boundary (optional)", value=float("nan"))
        density = st.checkbox("Scale y-axis to density (useful for comparing groups)", value=False)

        facet = st.selectbox("Facet by (optional categorical column)", options=["(none)"] + df.columns.tolist(), index=0)

        d = df[[x] + ([] if facet == "(none)" else [facet])].copy()
        if drop_na_rows:
            d = d.dropna()
        ...
        aes_kwargs = {"x": x}
        if density:
            # plotnine uses after_stat(density) similar to ggplot2
            p = ggplot(d, aes(x=x, y="after_stat(density)"))
            ylab = "Density"
        else:
            p = ggplot(d, aes(x=x))
            ylab = "Count"
        ...
        p = p + geom_histogram(**hist_kwargs) + labs(title=f"Histogram of {x}", x=x, y=ylab) + theme_bw()

        if facet != "(none)":
            # stacked histogram style via facets (like the chapter examples)
            p = p + facet_grid(rows=f"vars({facet})")
